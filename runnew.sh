...
set -e
#Start the container
sudo docker run --detach -v $(pwd)/templates/spark:/etc/spark -v $(pwd)/templates/minio:/etc/minio -v $(pwd)/templates:/templates -v ~/.kube/config:/etc/kubernetes/admin.conf -v $(pwd)/components/cvmfs.yaml:/etc/cvmfs_vals.yaml -v $(pwd)/tasks.conf:/etc/vc3/tasks.conf -v $(pwd)/task/HandleHeadNodes.py:/usr/lib/python2.7/site-packages/vc3master/plugins/task/HandleHeadNodes.py --rm --name vc3 virtualclusters/omnicontainer
...
#$sudo docker exec -i vc3 mkdir /etc/vc3
$sudo docker exec -i vc3 chmod 0755 /etc/vc3 
#$sudo docker exec -i vc3 mkdir 
